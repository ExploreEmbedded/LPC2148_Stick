...
const adcChannelConfig_st STR_AdcZeroConfig[C_MaxAdcZeroChannels_U8]=
{
        ...
        { P1_25, PINSEL_FUNC_1}, /* AD0[4] is on P1_25 Second alternative function */
...
const adcChannelConfig_st STR_AdcOneConfig[C_MaxAdcOneChannels_U8]=
{
...
void ADC_Init()
{
     uint32_t var_Pclk_u32;


    var_Pclk_u32 = M_GetClkDivValue();

    AD0CR = ((1<<SBIT_PDN) | var_Pclk_u32);  // no START, no BURST, ADC normal operation, with selected channel
    AD1CR = ((1<<SBIT_PDN) | var_Pclk_u32);  // no START, no BURST, ADC normal operation, with selected channel
}
...
uint16_t ADC_GetAdcValue(uint8_t var_adcChannel_u8)
 {
    uint16_t var_adcResult_u16;
    if(var_adcChannel_u8 <= AD0_7)
    {

       /* Select channel is with range, COnfigure the channel for ADC and DO the A/D conversion */
       GPIO_PinFunction(STR_AdcZeroConfig[var_adcChannel_u8].pinNumber,STR_AdcZeroConfig[var_adcChannel_u8].PinFunSel);
       AD0CR  = (AD0CR  & 0xFFFFFF00) | (0x01 << var_adcChannel_u8 );       /* set the channel */
       ...
       var_adcResult_u16 = (AD0GDR >> SBIT_RESULT) & 0x3ff; /*Read the 10bit adc result*/
    }
    else if((var_adcChannel_u8 >= AD1_0) && (var_adcChannel_u8<= AD1_7))
    {
        var_adcChannel_u8 = var_adcChannel_u8 - AD1_0;

        /* Select channel is with range, COnfigure the channel for ADC and DO the A/D conversion */
        GPIO_PinFunction(STR_AdcOneConfig[var_adcChannel_u8].pinNumber,STR_AdcOneConfig[var_adcChannel_u8].PinFunSel);
        AD1CR  = (AD1CR  & 0xFFFFFF00) | (0x01 << var_adcChannel_u8 );       /* set the channel */
        ...
        var_adcResult_u16 = (AD1GDR >> SBIT_RESULT) & 0x3ff; /*Read the 10bit adc result*/
    }
    else
    {
    ...
    return(var_adcResult_u16);                                  /* Return the 10-bit result */
 }
